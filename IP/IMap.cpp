...
int main(int argc, char **argv)
{
  ...
  int a = random_int(&seed, 1,5)*7;
  int b = random_int(&seed, 1,5)*5;
  int c = random_int(&seed, 1,5)*3;

  int va = random_int(&seed, 1,1000);
  int vb = random_int(&seed, 1,1000);
  int vc = random_int(&seed, 1,1000);

	IMap<int>::put(imaps, 0, a, va);
	IMap<int>::put(imaps, 1, b, vb);
	IMap<int>::put(imaps, 2, c, vc);

	printf("%d %d\n", a, (*imaps)[0].get(a).get(va));
	printf("%d %d\n", b, (*imaps)[1].get(b).get(vb));
	printf("%d %d\n", c, (*imaps)[2].get(c).get(vc));

  va = random_int(&seed, 1,1000);
  vb = random_int(&seed, 1,1000);
  vc = random_int(&seed, 1,1000);
  ...
  IMap<int>::put(*imaps, 2, c, vc);
	...
	IMap<int>::put(*imaps, 0, a, 0);
	IMap<int>::put(*imaps, 1, b, 0);
	IMap<int>::put(*imaps, 2, c, 0);
